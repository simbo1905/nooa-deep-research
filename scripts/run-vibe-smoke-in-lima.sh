#!/usr/bin/env bash
# Start a deliberately non-destructive Vibe -> Claude two-pack inside a
# detached tmux driver. Vibe is the short-lived programmatic sender; Claude
# stays as the persistent receiver and cleanup owner.
set -euo pipefail
...
limactl shell nooa-swarm -- bash -lc '
  ...
  tmux new-session -d -s nooa-vibe-smoke \
    "SWARMFORGE_TERMINAL=none SWARMFORGE_PREVENT_SLEEP=0 SWARMFORGE_INSTRUCTION_PROFILE=role-only SWARMFORGE_CONFIG=smoke-vibe.conf ./swarm > /tmp/nooa-vibe-smoke.log 2>&1"
